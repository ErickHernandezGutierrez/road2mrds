...
DIRS_FILENAME   = sys.argv[3]
DBF_FILENAME    = sys.argv[4]

d = load_dirs(DIRS_FILENAME)
ndirs = d.shape[0]
try:
    directions = open('directions.txt', 'wt')
    for dir in d:
        directions.write('%lf %lf %lf\n' % (dir[0], dir[1], dir[2]))
    directions.close()
except:
    print('error archivo')#"""
...
for i in range(N):
    for j in range(ndirs):
        Rj = getRotationFromDir( (0,1,0), d[j])
...
ax.plot(d[:,0], d[:,1], d[:,2], linewidth=0, marker='o', markersize=2)
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
